...
public class FileOperationAcl {
    private File file;

    public FileOperationAcl(File file){
        this.file = file;
    }

    //read file to get user's information, return Arraylist user
    public ArrayList<UserAcl> read(){
        ...
        try{
            ...
            while ((tmpLine = breader.readLine()) != null){
                ...
                UserAcl user = new UserAcl(userInfo[0], userInfo[1], Integer.parseInt(userInfo[2]), Integer.parseInt(userInfo[3]));
    ...
    public boolean deleteLine(String username){
        ...
        String str;
        for (UserAcl tmp : userArrayList){
            str = tmp.getUserName() + " " + tmp.getPwd() + " " + tmp.getGroup() + " " + tmp.getPermission() + System.lineSeparator();
            this.write(str);
        }
        return flag;
    }
